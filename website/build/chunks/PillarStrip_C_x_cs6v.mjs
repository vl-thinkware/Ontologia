...
const $$PillarStrip = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section class="py-14 md:py-28" style="background: var(--color-background);"> <div class="container-page"> ${(eyebrow || title) && renderTemplate`<header class="mx-auto max-w-2xl text-center mb-10 md:mb-14"> ${eyebrow && renderTemplate`<p class="text-eyebrow">${eyebrow}</p>`} ${title && renderTemplate`<h2 class="text-headline mt-3">${title}</h2>`} </header>`} <ul class="grid grid-cols-1 gap-5 md:grid-cols-3" role="list"> ${pillars.map((p, i) => {
...
}, "/sessions/bold-blissful-hawking/mnt/Ontologia/website/src/components/PillarStrip.astro", void 0);
